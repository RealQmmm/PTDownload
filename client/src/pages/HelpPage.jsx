...
const HelpPage = () => {
    ...
    const sections = [
        ...
        {
            title: '数据库配置 (Database Configuration)',
            icon: '🗄️',
            content: [
                '系统支持两种数据库存储方式，通过 Docker 卷映射自动检测，无需手动配置环境变量。',
                '内置数据库（默认）：数据存储在容器内的 /data 目录，适合快速部署和单机使用。不配置外部映射时自动使用。',
                '外部数据库（推荐）：数据存储在用户指定的外部路径（如 NAS），便于数据迁移，无需备份导入。配置外部映射后自动使用。',
                'Docker Compose 配置：在 volumes 中添加 "- /your/path:/external_db" 即可启用外部数据库，系统会自动检测并使用。',
                'NAS 图形界面配置：在容器设置中添加卷映射，宿主机路径填写实际路径（如 /Container/PTdownload），容器内路径固定填写 /external_db。',
                '数据迁移：迁移时需同时复制数据库文件（ptdownload.db）和加密密钥文件（.secret），两个文件必须配套使用。',
                '导出功能：在"备份与恢复"页面可以导出完整的数据库文件，便于迁移到外部存储或备份。'
            ]
        }
    ];
